...
CAT6 cabling running to the basement, but they weren't in ideal locations. I'm renting, so I don't have full access to the basement, and the house doesn't have an attic, so I needed to take the easy way out
...
## Why not fiber?
- I'm not planning on a network connection greater than 10Gbps, even on LAN, in the near to distant future, and 10GBASE-T does exist (but can be expensive and hot)
- Fiber transceivers are also expensive, and my PC already has a 2.5Gbps 8P8C port.
- Nobody sells white fiber optic cable unless you get a custom order (a big issue)
- Even though switches and cables are cheap, you'll probably end up spending more than with twisted pair on all the little things you need when just doing a single run.
...
- Future proofing? My Brocade ICX 6610 network switch does have a 40Gb QSFP port that I can easily take advantage of with my single-mode fiber.

# Cabling
In order for my cabling to blend in along the baseboards, it needed to be white. This is a super common color for CATx cabling - but not for fiber. Since fiber optic cables are often color coded to
...
{{ figure(src="fiber-optic-patch-cables.jpg", alt="alt text", caption="They're really not the best colors if you want the cable to blend in. Source: FS.com", width="300", height="300") }}

Then I remembered that I just moved, and my ISP, Bell, had ran a long fiber patch cable from one wall of my house where the fiber line comes in, to the other where the modem was. Could I use that cabling?
...
with "traditional copper wire installation processes", and it's crazy the fiber is able to survive all of this abuse and still have a strong enough signal come out the end.

## Connectors 
Fiber optic cables have many different [connector types](https://www.qsfptek.com/qt-news/apc-upc-pc-fiber-connector-types-differences-and-selection.html).
The Bell cable I have uses SC/APC connectors, but most SFP/SFP+ transceivers use LC/UPC connectors, which aren't compatible. So I also had to buy some [SC/APC couplers](https://www.fs.com/products/76106.html)
...
# NICs
I went with a cheap but reliable option (that still had modern Linux kernel support), an Intel X520 PCI-e NIC.
